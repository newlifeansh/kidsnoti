...
- 온보딩
  - `?firstVisit=1` 진입 시 기존 계정/자녀 데이터가 있어도 온보딩 프리뷰가 홈으로 튀지 않도록 수정
  - 중복되던 `AI 정리` 슬라이드 제거, 마지막 슬라이드에 `AI 정리 + 알림` 메시지를 통합
  - 마지막 슬라이드 미니 To-do 시각 영역 정리
- 가족 초대/공유
  - 공유 메시지 조사 처리 수정: `아내으로`가 아니라 `아내로`, `남편으로`처럼 자동 처리
  - 공유 딥링크 appName을 번들 설정과 동일한 `kidsnoti`로 수정
  - 기존 정보가 있는 계정이 초대 링크로 들어오면 즉시 가족 전환하지 않고 확인 팝업 노출
  - 확인 팝업에서 현재 저장된 아이/할 일/일정 수를 보여주고, `확인하고 합류`를 눌러야 초대 가족으로 전환
  - 실제 데이터는 물리 삭제가 아니라 현재 기본 가족 공간이 초대한 소유자 가족으로 전환되는 구조
- 홈/설정 UX
  - 홈 히어로의 아이 사진 클릭 시 해당 아이 프로필 수정 화면으로 바로 이동
  - 설정의 아이 상세 바텀시트에 `수정하기` 액션 추가
  - 가족 초대 직접 입력 input 우측 치우침 수정
- QA 체크
  - `npx tsc --noEmit` 통과
  - `npm run lint` 통과
  - 브라우저에서 온보딩 3단계, 마지막 슬라이드, 홈 사진 -> 프로필 수정 이동 확인
- Supabase / Admin 체크
  - 이번 수정은 프론트 수락 UX/딥링크/공유 문구 중심
  - Supabase SQL/RPC/Edge Function 파일 변경 없음
  - 기존 RPC `accept_family_invite`는 그대로 사용하며, 프론트에서 수락 전 확인 단계를 추가
  - 운영자 admin은 별도 과제로 유지, 이번 main 배포 대상 변경 없음
